...
overview: "Configurar uma função Netlify em https://hubspot-callback.netlify.app/nvoip-oauth-callback que replica o callback OAuth da Nvoip e retorna o usuário ao HubSpot usando as credenciais fornecidas."
todos: []
---
...
- Criar e documentar a função Netlify `/nvoip-oauth-callback` em `https://hubspot-callback.netlify.app` de modo que o HubSpot receba o callback completo com `state`, `code`, `error` e `nvoip_*`.
...
2. **hubspot-app-config** — Atualizar `src/app/settings/nvoipAuth.ts` e `src/app/app-hsmeta.json` para:
   ...
   - Usar `https://hubspot-callback.netlify.app/nvoip-oauth-callback` em `REDIRECT_URI` e `FALLBACK_REDIRECT`.
   - Liberar o domínio `hubspot-callback.netlify.app` em `permittedUrls.fetch` e `permittedUrls.iframe`.
...
5. **test-deploy** — Validar o fluxo final:
   ...
   - Fazer `hs project upload` e garantir que o HubSpot consiga redirecionar para `https://hubspot-callback.netlify.app/nvoip-oauth-callback`.
